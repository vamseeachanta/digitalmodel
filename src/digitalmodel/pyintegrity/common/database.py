...
def get_db_properties_for_service(service):
    ...
    db_properties.update(database_dict['Common'])
...
class Database():
    ...
    def save_to_db(self, df, table_name=None, if_exists='append', index=False, cfg={}):
        ...
        index = cfg.get('index', index)

        engine_schema_tables = self.engine.table_names(schema)
        if self.engine.has_table(table_name) or table_name in engine_schema_tables:
        ...
        if len(df.columns) > 0 and len(df) > 0 and len(df) < chunk:
            df.to_sql(name=table_name, con=self.engine, schema=schema, if_exists=if_exists, index=index)
        else:
            import math
            ...
            for chuck_index in range(0, number_of_chunks):
                ...
                if chuck_index == 1:
                    df_temp.to_sql(name=table_name, con=self.engine, schema=schema, if_exists=if_exists, index=index)
                else:
                    df_temp.to_sql(name=table_name, con=self.engine, schema=schema, if_exists='append', index=index)
    ...
    def executeScriptsFromFile(self, filename, arg_array=[]):
        ...
        try:
            print("     .....Executing query: {}".format(sqlFile))

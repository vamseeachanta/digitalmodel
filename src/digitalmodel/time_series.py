...
def time_series(cfg):
    ...
    if (
        'custom_calculation_1' in analysis and
        analysis['custom_calculation_1']['flag']
    ):
        ts.perform_custom_calculation_1()

    if (
        'custom_calculation_2' in analysis and
        analysis['custom_calculation_2']['flag']
    ):
    ...
    t_end = datetime.now()
    print(
        "Time taken to process: {0} seconds".format((t_end - t_start).seconds)
    )
    return cfg

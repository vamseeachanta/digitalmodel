# Multi process is not working - error occurs. frozen_support()
import pytest
import deepdiff
import os
import sys
...
def run_bs7910(input_file, expected_result={}):
    ...
    obtained_result = expected_result = {}
...
@pytest.mark.skip(reason="Multi processing running into error")
...
@pytest.mark.skip(reason="Multi processing running into error")
def test_bs7910_circumferential():
    input_file = 'test_data/fracture_mechanics/fracture_mechanics_py_circumferential_parallel_process.yml'
    pytest_output_file = 'test_data/API579/app_API579_16in_gas_b318_pytest.yml'
    pytest_output_file = get_valid_pytest_output_file(pytest_output_file)
    expected_result = ymlInput(pytest_output_file, updateYml=None)

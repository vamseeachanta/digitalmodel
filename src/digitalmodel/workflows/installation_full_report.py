...
def _cover(config, created):
    fields = [('Document', config.get('document_id', 'Installation engineering assessment')),
              ('Revision', config.get('revision', 'R7')), ('Issue purpose', config.get('issue_purpose', 'Technical review')),
    ...
    return (f'<header><div class="tag">Engineering assessment / technical review</div><h1>{escape(config.get("title", "Jumper installation engineering report"))}</h1>'
            '<p>Recorded demand, assumed-criteria envelopes and simulated monitoring</p></header>'
            ...
            '<h2>Contents</h2><ol>' + toc + '</ol><a href="#appendix-a">Appendix A: detailed results</a> · '

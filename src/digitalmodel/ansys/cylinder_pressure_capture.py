"""Read-only fixed N4 capture checks; no pressure numerical acceptance or launch."""
...
def inventory_capture(directory, deck_basename=CASE_ID+'.inp'):
    ...
    if deck_basename != CASE_ID+'.inp':
        raise ValueError('Only fixed N4 deck is in capture scope')
    ...
    for name in sorted(names):
        row, raw = _inventory_row(root, name)
        rows.append(row)
        if raw is not None:
            artifacts[name] = raw
...
def _equal_deck(raw, expected):
    if raw != expected:
        raise ValueError('Retained input differs from fixed N4 deck')
...
def _independent_checks(case, artifacts, execution, profile):
    checks = []
    native = artifacts.get(CASE_ID+'.out')
    _check(checks, 'deck', _equal_deck, artifacts.get(CASE_ID+'.inp'), case['deck_bytes'])
...
def capture_pressure(case, directory, execution, *, runtime_profile):
    ...
    if not isinstance(case, dict) or case.get('case_id') != CASE_ID:
        raise ValueError('Only fixed N4 pressure capture is admitted by this component')
    if not isinstance(execution, dict):
        raise ValueError('Execution claim must be a dictionary')
    fixed = build_case(CASE_ID)
    rows, artifacts = inventory_capture(directory)
    ...
    return dict(case_id=CASE_ID, capture_status='INCOMPLETE',
        ...
        reason='PRESSURE_PARSER_NOT_VALIDATED', engineering_qualified=False,

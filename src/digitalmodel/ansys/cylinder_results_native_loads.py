"""Observed v261 unloaded-control audit; pressure cases remain unsupported here."""
import re
...
def verify_zero_loads(raw, case):
    ...
    selection=re.fullmatch(rb'LIST CONSTRAINTS FOR SELECTED NODES 1 TO ([0-9]+) BY 1',rows[4])
    if (not selection or int(selection[1])!=max(n['node_id'] for n in case['nodes'])
            or rows[5]!=b'CURRENTLY SELECTED DOF SET= UX UY'):
        raise EvidenceError('Native DLIST selection differs')
    page_header(rows[6:10])
    if rows[10]!=b'NODE LABEL REAL IMAG' or rows[-1]!=b'FINISH SOLUTION PROCESSING':

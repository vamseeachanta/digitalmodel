...
class PressureVesselGenerator:
    ...
    def generate_pv_apdl(
        ...
        """Generate a complete, runnable APDL deck for pressure-vessel screening.

        Emits a self-contained static-structural solve of an **axisymmetric**
        cylindrical-shell wall section (PLANE182, KEYOPT axisymmetric) under
        internal pressure — the standard first-pass screening idealisation for a
        pressure-vessel wall (hoop + longitudinal membrane stress). The deck has
        a full ``/PREP7`` (element type, material, geometry, mesh), boundary
        conditions, ``/SOLU`` + ``SOLVE`` and a ``/POST1`` peak-von-Mises result
        with a small CSV digest, so the generated ``.inp`` runs end-to-end on a
        licensed MAPDL host via the run_ansys lane (#940/#948).

        Honesty: this is screening-grade — axisymmetric wall only, linear
        elastic, no nozzle/head discontinuities. The heuristic comment-blocks
        (geometry/thermal/wind/seismic/SCL generators) remain available as
        separate methods for building richer 3-D decks by hand; verify governing
        cases with a full model.
        ...
        include_thermal : bool
            Append a uniform-temperature thermal body load (default True).
        include_hydrotest : bool
            Use hydrotest pressure (1.3 x design unless overridden) instead of
            design pressure, and the UG-99 test allowable (yield / 1.5).
        ...
        """
        r_inner = geom.inner_diameter_mm / 2.0
        wall_t = geom.wall_thickness_mm
        r_outer = r_inner + wall_t
        # A short axial slice is enough for membrane screening; keep it square-ish.
        axial_len = max(wall_t * 4.0, 50.0)
        esize = max(wall_t / 4.0, 1.0)
        youngs_mpa = 200_000.0
        poisson = 0.3
        cte_per_c = 1.2e-5

        if include_hydrotest:
            if conditions.hydrotest_pressure_mpa is not None:
                pressure = conditions.hydrotest_pressure_mpa
            else:
                pressure = 1.3 * conditions.design_pressure_mpa
            allowable = conditions.yield_strength_mpa / 1.5  # UG-99 test allowable
        else:
            pressure = conditions.design_pressure_mpa
            allowable = conditions.allowable_stress_mpa

        delta_t = (
            conditions.operating_temperature_c - conditions.ambient_temperature_c
        )

        lines = [
            "! ============================================================",
            f"! Pressure-vessel screening FE — {conditions.material_name}",
            "! Generated by digitalmodel.ansys.pressure_vessel",
            "! Units: mm, N, MPa (axisymmetric cylindrical wall section)",
            "! ============================================================",
            "FINISH",
            "/CLEAR,NOSTART",
            f"/TITLE,PV screening: ID={geom.inner_diameter_mm}mm t={wall_t}mm "
            f"P={pressure}MPa",
            "/UNITS,MPA",
            "/PREP7",
            "",
            "! --- material (linear elastic steel) ---",
            f"MP,EX,1,{youngs_mpa}",
            f"MP,PRXY,1,{poisson}",
            f"MP,ALPX,1,{cte_per_c}",
            "",
            "! --- element: PLANE182, axisymmetric ---",
            "ET,1,PLANE182",
            "KEYOPT,1,3,1            ! axisymmetric",
            "",
            "! --- geometry: r-z wall section (X=radial, Y=axial) ---",
            f"RECTNG,{r_inner},{r_outer},0,{axial_len}   ! area 1 = wall slice",
            "",
            "! --- mesh ---",
            "TYPE,1",
            "MAT,1",
            ...
            "",
            "! --- boundary conditions ---",
            "! symmetry / axial restraint on the bottom edge (y=0). This is the",
            "! ONLY restraint required: an axisymmetric element has no radial",
            "! rigid-body mode (a uniform radial translation produces hoop strain,",
            "! so it is not a rigid-body motion), and UY here removes the single",
            "! axial mode. A radial point restraint would resist the wall's own",
            "! pressure and thermal expansion -- see #2094.",
            "NSEL,S,LOC,Y,0",
            "D,ALL,UY,0",
            "ALLSEL,ALL",
            "",
            "! --- load: internal pressure on the inner wall (x=r_inner) ---",
            f"! Design/test pressure = {pressure} MPa",
            f"LSEL,S,LOC,X,{r_inner}",
            f"SFL,ALL,PRES,{pressure}",
            "ALLSEL,ALL",
        ]
        ...
        if include_thermal:
            lines.extend(
                [
                    "",
                    "! --- thermal body load (uniform operating delta-T) ---",
                    f"! delta-T = {delta_t} C above ambient",
                    f"TREF,{conditions.ambient_temperature_c}",
                    f"BFUNIF,TEMP,{conditions.operating_temperature_c}",
                ]
            )
        ...
        lines.extend(
            [
                ...
                "FINISH",
                "",
                "! --- post: peak von Mises + ASME membrane unity check ---",
                "/POST1",
                "SET,LAST",
                "NSORT,S,EQV",
                "*GET,smax,SORT,0,MAX",
                "*GET,peak_node,SORT,0,IMAX   ! a value without its location cannot",
                "                             ! be told from a singularity",
                f"allow = {allowable}",
                "uc = smax / allow",
                "",
                "! --- equilibrium check (#2094) ---",
                "! No axial load is applied -- the pressure acts on the inner wall",
                "! only and there is no end cap -- so the axial reaction shall be",
                "! zero. A non-zero value means an unintended load or restraint.",
                "! A radial reaction cannot arise: nothing is restrained radially.",
                "NSEL,S,LOC,Y,0",
                "FSUM",
                ...
                "ALLSEL,ALL",
                "/COM,============================================================",
                "/COM,PRESSURE-VESSEL SCREENING RESULT",
                "*MSG,INFO,smax,allow,uc",
                "peak von Mises = %G MPa ; allowable = %G MPa ; UC = %G",
                "*MSG,INFO,rfx,rfy",
                "reaction FX = %G N ; reaction FY = %G N  (both shall be ~0)",
                "/COM,============================================================",
                "! numeric digest for the postprocessor (small, returnable)",
                "*CFOPEN,pv_result,csv",
                "*VWRITE,smax,allow,uc,peak_node,rfx,rfy",
                (
                    "('max_seqv_mpa,',F12.4,',allowable_mpa,',F12.4,',uc,',F10.5,"
                    "',peak_node,',F10.0,',reaction_fx_n,',F14.4,"
                    "',reaction_fy_n,',F14.4)"
                ),
                "*CFCLOS",
                "FINISH",

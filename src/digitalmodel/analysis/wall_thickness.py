...
class WallThicknessAnalyzer:
    ...
    def perform_analysis(self) -> WallThicknessResult:
        ...
        if self.code == DesignCode.DNV_ST_F101:
            u, d = self.check_pressure_containment()
            checks["pressure_containment"] = u
            details["pressure_containment"] = d

            u, d = self.check_collapse()
            checks["collapse"] = u
            details["collapse"] = d

            u, d = self.check_propagation_buckling()
            checks["propagation_buckling"] = u
            details["propagation_buckling"] = d

            u, d = self.check_combined_loading()
            checks["combined_loading"] = u
            details["combined_loading"] = d

        elif self.code == DesignCode.API_RP_1111:
            u, d = self.check_api_burst()
            checks["burst"] = u
            details["burst"] = d

            u, d = self.check_api_collapse()
            checks["collapse"] = u
            details["collapse"] = d

            u, d = self.check_api_propagation()
            checks["propagation"] = u
            details["propagation"] = d
        ...
        logger.info(
            f"Analysis complete ({self.code.value}): "
            f"governing={governing}, max_util={max_util:.3f}, "

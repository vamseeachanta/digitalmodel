...
class OrcInstallation:
    ...
    def create_model_for_water_depth(self, cfg):
        ...
        ref_model_file = OrderedDict(ymlInput(cfg_str['reference_model_file']))
        ref_elevation_file = OrderedDict(
            ymlInput(cfg_str['reference_elevation_file']))
        ...
        for delta_elevation in cfg_str['delta_elevations']:
            orc_object = '6DBuoys'
            self.update_elevation_for_orc_object(cfg_str, model_file,
                                                 ref_elevation_file,
            ...
            orc_object = 'Lines'
            self.update_lines_for_installation(cfg_str, model_file,
                                               ref_model_file, delta_elevation,
                                               orc_object)

            elevation_for_file_name = int(
                round(ref_elevation_file['6DBuoys'][cfg_str['6DBuoys'][0]]
                      ['InitialZ'] + delta_elevation))
            ...
            file_name = os.path.join(cfg['Analysis']['analysis_root_folder'],
                                     file_base_name)
            saveDataYaml(model_str_orientation,
                         file_name,
                         default_flow_style='OrderedDumper')
    ...
    def update_elevation_for_orc_object(self, cfg_str, model_file,
                                        ref_elevation_file, delta_elevation,
                                        orc_object):
        ...
        if orc_object in cfg_str or len(cfg_str[orc_object]) > 0:
            for buoy in cfg_str[orc_object]:
                ...
                model_file.update({
                    orc_object: {
                        buoy: {
                            'InitialZ': reference_elevation + delta_elevation
                        }
                    }
                })

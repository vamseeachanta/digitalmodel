class VerticalRiser():
    ...
    def read_riser_input_data(self):
        ...
        for riser_data_index in range(0, len(custom_cfg['files']['from_xlsx'])):
            ...
            setattr(self, attribute_name, read_data.from_xlsx(custom_cfg,riser_data_index))


    def assign_key_properties_to_self(self):
        self.host_id = self.cfg['default']['analysis']['host']['id']
        self.host_name = self.hosts[self.hosts['host_id'] == self.host_id]['host_name'].values[0]
        self.host_construction_type = self.hosts[self.hosts['host_id'] == self.host_id]['construction_type'].values[0]
        ...
        if (self.stack_up_table[self.stack_up_table['Component'] == 'Support Spring']['No. Of. Joints'].values[0] == 1):
            self.support_spring = True
        else:
            self.support_spring = False

        self.initial_down_stroke = self.cfg['stack_up']['stroke_setting']['initial_down_stroke']
        self.model_stroke_in_and_out_limits = self.cfg['stack_up']['stroke_setting']['model_stroke_in_and_out_limits']
    ...
    def delete_unwanted_joint_rows(self):
        ...
        self.riser_joints = self.riser_joints[(self.riser_joints['host_id'] == self.host_id) | (self.riser_joints['well_id'] == self.well_id)]
    ...
    def get_fea_properties(self, row_index, weight_method=None):
        ...
        property_dictionary['section_properties']['pipe'] = self.update_property_dictionary_to_si_units(property_dictionary['section_properties']['pipe'])

        cd_array = [self.riser_joints.iloc[row_index]['Lateral Drag Coefficient, ' + self.fea_type + ', Cd'], '~',
                            self.riser_joints.iloc[row_index]['Axial Drag Coefficient, ' + self.fea_type + ', Cd']]
        ca_array = [self.riser_joints.iloc[row_index]['Lateral Added mass, Ca'], '~', self.riser_joints.iloc[row_index]['Axial Added mass, Ca']]
        pipe_id = (self.riser_joints.iloc[row_index]['Pipe OD'] - 2 *self.riser_joints.iloc[row_index]['Pipe WT'])*0.0254

        if self.riser_joints.iloc[row_index]['Weight Method'] == 'measured':
            logging.info("Calculating based on measured properties")
            dry_weight_per_unit_length = self.riser_joints.iloc[row_index]['Dry wt.'] * 0.4536 * 1000 / (
                        self.riser_joints.iloc[row_index]['Length'] * 0.3048)
            wet_weight_per_unit_length = self.riser_joints.iloc[row_index]['Wet wt.'] * 0.4536 * 1000 / (
                        self.riser_joints.iloc[row_index]['Length'] * 0.3048)
            buoyancy_diameter = math.sqrt((dry_weight_per_unit_length-wet_weight_per_unit_length)*4/math.pi/self.cfg['Material']['SeaWater']['Rho']
                                          +  pipe_id **2)
            ...
            dry_weight_per_unit_length = property_dictionary['section_properties']['pipe']['MassPerUnitLength'] * 0.4536 / 0.0254
            ...
            logging.info("Properties can not evaluated for below jt values row as *Weight Method* not available")
        ...
        effective_ID_with_axiliary_lines = math.sqrt(4/math.pi*(property_dictionary['section_properties']['pipe']['Ai'] + self.auxiliary_lines_properties['Ai']))
        property_dictionary['section_properties']['pipe'].update({
    ...
    def prepare_flexible_joint_data(self):
        import logging
        self.project_flexible_joints = self.flexible_joints[(self.flexible_joints['host_id'] == self.host_id) &
    ...
    def prepare_tensioner_data(self):
        import logging
        self.project_tensioners = self.tensioners[(self.tensioners['host_id'] == self.host_id)]
    ...
    def prepare_geotechnical_data(self):
        import logging
        self.project_geotechnical = self.geotechnical[(self.geotechnical['well_id'] == self.well_id) &
                             (self.geotechnical['application_type'] == self.geotechnical_application_type)]

        self.project_geotechnical = self.project_geotechnical.sort_values(by=['depth_below_seabed', 'y']).copy()
    ...
    def calculate_auxiliary_line_properties(self):
        ...
        auxiliary_lines = self.riser_joints[self.riser_joints['Joint Type']== 'auxiliary_line']
        ...
        for joint_item_index in range(0, len(auxiliary_lines)):
            for properties_index in range(0, len(self.pipe_properties)):
                if auxiliary_lines.loc[joint_item_index, 'Component'] == self.pipe_properties[properties_index]['section_properties']['pipe']['Name']:
                    auxiliary_lines_internal_area = auxiliary_lines_internal_area + self.pipe_properties[0]['section_properties']['pipe']['Ai']
    ...
    def save_data(self):
        file_name = ({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
            'file_name'] + '_' + '{0}'.format('stack_up_properties') + '.csv'})
    ...
    def prepare_plots(self):
        ...
        for plot_settings_index in range(0, len(self.cfg['plot_settings'])):
            ...
            plt_settings.update({'file_name': self.cfg['Analysis']['result_folder'] + self.cfg['Analysis'][
                'file_name'] + '_' + '{0}'.format(plt_settings['file_name']) + '.png'})
    ...
    def alter_properties_for_Shear7_software(self):
        minimum_fea_segment_length = self.stack_up_table['FEA Segment Length'][1:len(self.stack_up_table)].min()

...
Clean version - only uses Excel data, no external configurations.
"""
...
def extract_vessel_properties(excel_path: str, draft_ft: float = None) -> List[Dict[str, Any]]:
    ...
    # 2 configurations from the Excel file
    config_data = [
        {
            'name': 'upright',
...
def main():
    ...
    for vessel in vessels:
        ...
        print(f"  Mass: {mass:.4f} kg ({vessel['mass_properties']['mass']['original']['value']:.4f} LT)")
        ...
        print(f"  Gyradii: ({vessel['mass_properties']['radii_of_gyration']['kxx']['value']:.4f}, "
              f"{vessel['mass_properties']['radii_of_gyration']['kyy']['value']:.4f}, "
              f"{vessel['mass_properties']['radii_of_gyration']['kzz']['value']:.4f}) m")

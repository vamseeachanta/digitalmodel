...
<style>
:root{--burst:#0072b2;--collapse:#d55e00;--combined:#009e73;--propagation:#cc79a7;
--other1:#e69f00;--other2:#56b4e9;--other3:#6f4e9c;--other4:#8c564b}
...
let plotMode="canvas",requestId=0,allow3d;
 ...
 for(let i=0;i<nu-1;i++)for(let j=0;j<nb-1;j++){const id=(x,y)=>({x,y,u:um[y][x]});
  for(const tri of [[id(i,j),id(i+1,j),id(i+1,j+1)],[id(i,j),id(i+1,j+1),id(i,j+1)]]){
   const hits=[];for(const [a,b] of [[tri[0],tri[1]],[tri[1],tri[2]],[tri[2],tri[0]]])
    if((a.u<level&&b.u>=level)||(b.u<level&&a.u>=level))hits.push(point(a,b));
   if(hits.length>=2)out.push([hits[0],hits[1]]);}}return out;}
...
function nativePlot(um,gm){const box=canvas.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2);
 ...
 c.fillStyle=SOFT;c.fillRect(0,0,w,h);const pw=w-L-R,ph=h-T-B,nu=M.t.length,nb=M.b.length;
 for(let i=0;i<nu-1;i++)for(let j=0;j<nb-1;j++){c.fillStyle=COLORS[gm[j][i]%COLORS.length];
...
function traces2d(um,gm){const text=gm.map(row=>row.map(i=>label(DATA.g[i])));
 return[{type:"heatmap",x:M.t.map(v=>v/1e6),y:M.b.map(v=>v/1e3),z:gm.map(r=>r.map(v=>v+.5)),
  customdata:um,text,zmin:0,zmax:DATA.g.length,colorscale:discreteScale(),showscale:false,
...
function traces3d(um,gm){const x=[],y=[],z=[],nu=M.t.length,nb=M.b.length;
 ...
 const groups=DATA.g.map(()=>({i:[],j:[],k:[]})),pick=ids=>{const count={};
  ids.forEach(id=>count[gm[id%nb][Math.floor(id/nb)]]=(count[gm[id%nb][Math.floor(id/nb)]]||0)+1);
 ...
 for(let i=0;i<nu-1;i++)for(let j=0;j<nb-1;j++){const a=i*nb+j,b=(i+1)*nb+j,c=b+1,d=a+1;
  for(const tri of [[a,b,c],[a,c,d]]){const group=groups[pick(tri)];group.i.push(tri[0]);group.j.push(tri[1]);group.k.push(tri[2]);}}
...
async function plotlyPlot(mode,um,gm){if(!window.Plotly)return;const id=++requestId;
 try{const traces=mode==="3d"?traces3d(um,gm):traces2d(um,gm),fn=plot._fullLayout?"react":"newPlot";
  ...
  if(id!==requestId)return;plot.style.display="block";canvas.style.display="none";plotMode=mode;
  document.getElementById("mode").textContent=mode==="3d"?"3D WebGL surface.":"2D fallback — WebGL is unavailable.";
 }catch(error){if(mode==="3d"){allow3d=false;plotMode="2d";await plotlyPlot("2d",um,gm);}else{
  plot.style.display="none";canvas.style.display="block";
...
function render(){const [um,gm]=matrices();nativePlot(um,gm);updateText(um,gm);
 if(window.Plotly)plotlyPlot(allow3d?"3d":"2d",um,gm);}

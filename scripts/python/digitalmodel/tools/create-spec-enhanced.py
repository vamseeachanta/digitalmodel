...
ALL document processing during enhanced spec creation MUST use the MarkItDown MCP server:
...
- Executive Summary: Include document conversion metrics and processing notes
"""
...
def create_enhanced_spec_directory(spec_name, module_name):
    ...
    specs_base = Path("specs/modules")  # Changed from .agent-os/specs/modules
...
def main():
    ...
    if len(sys.argv) < 3:  # Changed from < 2 to < 3 - module is now required
        ...
        print("  python create-spec-enhanced.py data-analysis research-module research")
        print("  python create-spec-enhanced.py bug-fix core minimal")
        return 1

#!/usr/bin/env python
"""
Test processing with combined vessel/jacket plots and optimized y-axis range
"""
...
def process_single_test_file():
    """Process a single fatigue CSV file with combined plots"""
    ...
    try:
        ...
        # 2. COMBINED FFT Spectrum with optimized y-axis
        ax2 = plt.subplot(2, 3, 2)
        freq_limit = 2.0
        colors_fft = ['darkgreen', 'darkred']
        ...
        for col_name, spectrum in fft_data.items():
            if isinstance(spectrum, pd.DataFrame):
                ...
                all_powers.extend(powers[powers > 0])  # Only positive values for log scale
        
        if all_powers:
            # Set y-axis limits based on data range
            y_min = np.percentile(all_powers, 5)  # 5th percentile
            y_max = np.percentile(all_powers, 99.5)  # 99.5th percentile
        ...
        for idx, (col_name, spectrum) in enumerate(fft_data.items()):
            if isinstance(spectrum, pd.DataFrame):
                ...
                ax2.semilogy(spectrum.loc[mask, 'frequency'], 
                           ...
                           alpha=0.8, color=colors_fft[idx], linewidth=1.5,
                           label=label)
        ...
        # 3. COMBINED Welch PSD with optimized y-axis
        ax3 = plt.subplot(2, 3, 3)
        colors_psd = ['darkblue', 'darkorange']
        ...
        for idx, (col_name, psd) in enumerate(psd_data.items()):
            if isinstance(psd, pd.DataFrame):
                ...
                ax3.semilogy(psd.loc[mask, 'frequency'],
                           psd.loc[mask, 'power'],
                           alpha=0.8, color=colors_psd[idx], linewidth=1.5,
                           label=label)
        ...
        for idx, col_name in enumerate(data_columns):
            if col_name in df.columns:
                ...
                ax4.plot(time_plot, df[col_name].values[:sample_points], 
                        label=label, alpha=0.7, linewidth=1, color=colors_time[idx])
        ...
        info_text += "NOTE:\n"
        info_text += "Remaining peaks after smoothing\n"
        info_text += "represent actual system response\n"
        info_text += "characteristics important for\n"
        info_text += "fatigue analysis."
        ...
        # Summary
        logger.info("\n" + "="*60)
        logger.info("COMBINED ANALYSIS RESULTS")
        logger.info("="*60)
        ...
        logger.info(f"  - Plots: Vessel End and Jacket End combined")
        ...
        logger.info(f"  - {plot_filename} (combined plots)")
        
        logger.info("\n✅ Vessel End and Jacket End combined in all plots")

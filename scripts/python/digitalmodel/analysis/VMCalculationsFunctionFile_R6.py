...
def VMStress():
    ...
    from VMStressCalculations_R11 import pipe
...
##    Moment_LHS = 2*(Moment/(2*pipeI(pipe.pipeNominalOD_m-pipe.pipeMinimumWT_m)))**2+2*Moment*(((1/((2*pipeI)*(pipe.pipeNominalOD_m-pipe.pipeMinimumWT_m))*(sigmaRadial_1)))-(sigmaCircuferential_1*(1/((2*pipeI)*(pipe.pipeNominalOD_m-pipe.pipeMinimumWT_m)))))
    ...
    Moment = 0*1E-3, Moment_positive*1E-3, 0*1E-3, Moment_negative*1E-3, 0*1E-3
    Tension = Tension_negative*1E-3,0*1E-3, Tension_positive*1E-3, 0*1E-3, Tension_negative*1E-3
    ...
    pyplot.ylabel('Bending Moment (kNm)', fontsize=12, fontweight='bold', color='black') 
    #it creates plot Y-axis name, fontsize and colour             
    pyplot.xlabel('Effective Tension  (N)', fontsize=12, fontweight='bold', color='black')
    #it converted Static max.effective tension value KN to Te and it plots these values along with arc length
    pyplot.plot(Tension, Moment)
    pyplot.savefig("TensionvsMoment.png",dpi=800)
    pyplot.show()

...
ALLOWLIST = {
    ...
    # Generated by scripts/cfd/generate-kcs-verification.py as a self-contained
    # calc report: it is emailed and read offline, so it carries its own theme
    # rather than linking ../_assets/brand.css, which would not resolve outside
    # the docs tree. Pre-existing violation, allowlisted per this guard's own
    # instruction rather than left to block unrelated work.

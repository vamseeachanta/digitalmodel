...
def main() -> None:
    ...
    print(f"design grids: {n} combos "
          ...
          f"x {len(data['spacings'])} spacings)")
    print(f"thickness {data['thickness'][0]}-{data['thickness'][-1]} mm, "
          f"load {data['load'][0]}-{data['load'][-1]} MPa, span "
          f"{data['meta']['plate_length_mm']:g} mm, gamma_m {data['meta']['gamma_m']}")
...
<footer>
  ...
  &gamma;<sub>M</sub> = {data['meta']['gamma_m']}, frame span {data['meta']['plate_length_mm']:g} mm.
...
function key(){{ return $('grade').value + '|' + $('profile').value + '|' + $('spacing').value; }}
...
function render(){{
  ...
  const layout = {{
    ...
    title:{{text:'Design window — '+$('grade').value+' · '+$('profile').value+' · '+$('spacing').value+' mm spacing', x:0.02, font:{{size:16}}}},
    xaxis:{{title:'Plate thickness (mm)'}},
    yaxis:{{title:'Applied longitudinal load &sigma;<sub>x</sub> (MPa)', rangemode:'tozero'}},
...
function probeAt(t, load){{
  ...
  ro.innerHTML =
    '<b>'+$('grade').value+' · '+$('profile').value+' · '+$('spacing').value+' mm</b> &nbsp;|&nbsp; '
...
['grade','profile','spacing','colorby'].forEach(id => $(id).addEventListener('change', () => {{ probe=null; resetReadout(); render(); }}));

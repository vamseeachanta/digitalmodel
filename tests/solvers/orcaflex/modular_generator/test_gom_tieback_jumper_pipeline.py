"""Regression tests for the GoM tieback spec-to-modular pipeline (#602, #603).
...
CI command:
    uv run pytest tests/solvers/orcaflex/modular_generator/test_gom_tieback_jumper_pipeline.py -q
"""
...
GOM_TIEBACK = ["gom_tieback_mf_plet", "gom_tieback_plet_plem"]
...
@pytest.mark.parametrize("name", GOM_TIEBACK)
...
def test_configurations_currently_identical_pending_workbook():
    ...
    a = build_modular_spec(str(_spec_path("gom_tieback_mf_plet")))
    b = build_modular_spec(str(_spec_path("gom_tieback_plet_plem")))

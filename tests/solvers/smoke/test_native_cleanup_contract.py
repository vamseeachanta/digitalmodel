...
def test_unconfirmed_cleanup_cannot_return_success(tmp_path, cleanup_wait):
    ...
    namespace["active_processes"] = lambda *args: 0
    clock = iter([0, 11])
    namespace["time"] = SimpleNamespace(monotonic=lambda: next(clock), sleep=lambda _: None)
    waits = iter([0, cleanup_wait])
    namespace["_winapi"] = SimpleNamespace(
        ...
        TerminateProcess=lambda *args: None,
    )

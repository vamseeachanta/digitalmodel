...
# ABOUTME: Tests validated against textbook answers from 6 chapters (24 tests)
"""
...
Modules implemented: fundamentals, hydrostatics, integration,
    stability, resistance, submarines
Modules pending: curves_of_form (needs ship hydrostatic data tables)
...
class TestEN400Ch2HullFormGeometry:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                # Lpp=326.4 ft, 5 stations, spacing=81.6 ft
                [0.39, 12.92, 20.97, 21.71, 12.58],
                81.6,
                10523.0,
    ...
    def test_simpsons_first_rule_area(
        ...
        area = 2 * half_area
        assert abs(area - expected_area_ft2) < 10.0
...
class TestEN400Ch3Hydrostatics:
    ...
    @pytest.mark.xfail(
        reason="curves_of_form needs ship hydrostatic data tables"
    )

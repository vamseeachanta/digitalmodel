"""Final test of the AQWA parser fix."""
...
def test_final():
    ...
    try:
        ...
        if len(found_missing) == len(missing_headings):
            ...
            print(f"\nISSUE: Still missing some headings. Found: {found_missing}")
            
        # Show sample data for verification
        if len(result['frequencies']) > 0 and len(result['headings']) > 0:
            freq_idx = 0
            heading_idx = result['headings'].tolist().index(-135) if -135 in result['headings'] else 0
            
            print(f"\nSample data for frequency {result['frequencies'][freq_idx]}, heading {result['headings'][heading_idx]}:")

...
from dataProvision.VMStressCalculations_R14 import *
...
##    DesignCaseFac = 1 #Cf #Taken from Section4.4 Table-2 under Load Category-Survival
    ...
    sigmaRadial_1 = -((pipe.pipeExternalPressure*pipe.pipeNominalOD_m)+(pipe.pipeInternalPressure*pipeNominalID))/(pipe.pipeNominalOD_m+pipeNominalID)
    print("sigmaRadial_1 : ","{:.3f}".format(sigmaRadial_1))

    sigmaCircuferential_1 = (((pipe.pipeInternalPressure-pipe.pipeExternalPressure)*pipe.pipeNominalOD_m)/(2*pipe.pipeMinimumWT_m))-pipe.pipeInternalPressure
    ...
    count = 0
    x = range(8)
    list1 = [pipe, pipe2,pipe3,pipe4,pipe5,pipe6,pipe7,pipe8]
    TensionList1 = []
    TensionList2 = []
    BendMomentList = []
    for i in list1:

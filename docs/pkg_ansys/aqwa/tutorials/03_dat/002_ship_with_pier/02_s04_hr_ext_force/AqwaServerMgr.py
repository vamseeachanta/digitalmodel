...
class AqwaServerMgr:
    def __init__(self,port=0):
        ...
        AQWA_SocketUserForceServerDetailscfg = os.path.join(dir_path, "AQWA_SocketUserForceServerDetails.cfg")
...
class AqwaUserForceServer(AqwaServerMgr):
    ...
    def initiate_result_df(self):
        columns = ['TimeStep', 'Y_Coordinate', 'delta_L', 'stiffener_force', 'velocity', 'dampener_force', 'total_force']
        self.Analysis.result_df = pd.DataFrame(columns=columns)

    def save_result_df(self):
        InputFileName = self.Analysis.InputFileName.split('/')[-1]
        self.Analysis.result_df.to_csv(InputFileName + '.csv', index=False)

    def Run(self,UserForceFunction,ClosingFuntion=None):

        self.BaseReset()
        self.Start()
        self.initiate_result_df()

...
def VMStress():
    ...
    from VMStressCalculations_R8 import pipe
...
##    print("LHS_Squared_2 : ","{:.3e}".format(LHS_Squared_2))
    ...
    trueWallTensionLimit = DesignCaseFac*pipe.pipeYieldStrength*pipeA
    print("Tension Limit in kN : ","{0:.2f}".format(trueWallTensionLimit*1E-3))

    bendingMomentLimit = DesignCaseFac*pipe.pipeYieldStrength*2*pipeI/(pipe.pipeNominalOD_m - pipe.pipeNominalWT_m)
    print("Bending Moment Limit in kN.m : ","{0:.2f}".format(bendingMomentLimit*1E-3))

    internalPressureLimit = (DesignCaseFac*pipe.pipeYieldStrength*math.sqrt(2))/math.sqrt((pipeNominalID/(pipeNominalID + pipe.pipeNominalOD_m))**2 +(pipe.pipeNominalOD_m/2/pipe.pipeNominalWT_m)**2 +((pipeNominalID/(pipeNominalID + pipe.pipeNominalOD_m))-(pipe.pipeNominalOD_m/2/pipe.pipeNominalWT_m) )**2 )

...
RiserStretch=(TensionerEndBZHalf-min(TensionerEndBZConstant1))/0.3048+max(TensionerLengthConstant2-TensionerLengthConstant1)/0.3048
#It creates new csv file
b = open('RiserStretch_TensionStroke.csv', 'w', newline='')
#It allows writing on csv file
a = csv.writer(b)
#It gives data
TensionStroke = Times1, TensionerStrokeComplete
Vessel_Heave = VesselHeave
Riser_Stretch = Times3, RiserStretch 
#It creates row on that data
a.writerows(TensionStroke)
a.writerow(Vessel_Heave)
a.writerows(Riser_Stretch)
b.close()

...
- `tests/solver/smoke_test.py` demonstrates `OrcFxAPI.DLLVersion()` only in its `__main__` block; the pytest test bodies do not cover doctor-style version diagnostics.
...
- Current runner code checks OrcFxAPI importability but does not call `DLLVersion()`; `tests/solver/smoke_test.py` demonstrates `DLLVersion()` use only outside pytest. No OrcFxAPI license-check API has been verified, and repo grep found no source-backed `orcawave.exe -check-license` or equivalent command at current HEAD. API and executable license readiness therefore remain `UNKNOWN` in #613; a future issue must define and review any active license probe before the doctor reports license `PASS`/`FAIL`.
...
1. Add a diagnostics service that checks OrcFxAPI import, DLL/version when available, API license status (`UNKNOWN` because no verified OrcFxAPI license API exists in current repo evidence), executable license status (`UNKNOWN` because no verified executable license command exists in current repo evidence), executable candidates, `ORCAWAVE_PATH`, configured `--executable`, output writability/creatability, platform, thread count, and basic memory/runtime guidance. Current runner code checks importability but does not call `DLLVersion()`; the version check is new doctor behavior and must not be described as existing runner behavior. #613 must not claim `orcawave.exe -check-license` or any equivalent executable license probe is supported. `--require-license` exits nonzero because the requirement cannot be proven while license status remains `UNKNOWN`. Active API/executable license probing is deferred to a future issue that first identifies a source-backed command/API and gets its own plan review.
...
5. Define JSON schema: `{schema_version, overall_status, checks:[{name,status,message,details}], execution_mode, exit_code}` where status is `PASS|UNKNOWN|WARN|FAIL`. `overall_status` uses the total ordering `FAIL > WARN > UNKNOWN > PASS`, even when no requirement flag was requested; `UNKNOWN` is advisory but must still surface above all-pass readiness because the license cannot be proven. `exit_code` is nonzero only when a requested requirement is unmet or a CLI validation error occurs. This allows a JSON report to contain a `FAIL`, `WARN`, or `UNKNOWN` readiness check and an exit code of 0 when the user asked for advisory diagnostics only. `execution_mode` values are `api_available`, `executable_available`, `dry_run_only`, and `unknown`; priority follows runner intent: API available wins when `use_api` is true, else usable executable, else dry-run-only.
...
orcawave_doctor(options):
  checks = [
    ...
    check_api_license_unknown(),
    check_executable_license_unknown(),
  ...
  overall = worst check status, with FAIL taking precedence even for advisory checks
  render human or JSON schema_version=1
  exit 1 only when a requirement is unmet or CLI option validation failed
```
...
| Plan index | N/A in the nested `digitalmodel` repo - `digitalmodel/docs/plans/README.md` does not exist and this nested repository does not maintain the parent workspace `/mnt/local-analysis/workspace-hub/docs/plans/README.md` index for repo-local plans; issue comment links are the required discoverability record for this plan |
...
|---|---|---|---|
| `test_orcawave_doctor_no_orcfxapi_reports_dry_run_only` | Linux fallback clear | import unavailable and every executable source mocked/cleared (`--executable`, `ORCAWAVE_PATH`, standard install paths, and `PATH`/`shutil.which`) | WARN text, exit 0 by default, no stack trace |
...
| `test_orcawave_doctor_license_unknown_without_verified_api` | no false license claim | OrcFxAPI version available but no license API | API license check status `UNKNOWN`; overall status follows `FAIL > WARN > UNKNOWN > PASS`; `--require-license` exits nonzero |
| `test_orcawave_doctor_executable_license_unknown_without_verified_command` | unsupported executable license command is not guessed | usable executable exists but no source-backed license probe command exists in current repo evidence | executable license check status `UNKNOWN`; no command is executed; `--require-license` exits nonzero |
| `test_orcawave_doctor_require_api_exit_policy` | hard requirement applied | `--require-api` and no OrcFxAPI | nonzero exit |
| `test_orcawave_doctor_require_executable_exit_policy` | executable requirement applied | `--require-executable` and no usable executable candidate | nonzero exit and failed executable requirement in JSON |
...
| `test_orcawave_doctor_output_dir_existing_file_fails` | non-directory output target matches runner hard failure | `--output-dir` path that exists as a file or where a component is a file | check status and `overall_status` are `FAIL` by default, exit remains 0 unless `--require-output-writable` is set, and no filesystem mutation occurs |
...
- [ ] OrcFxAPI license status is `UNKNOWN` because no verified API license check exists in current repo evidence; the doctor must not imply a license is available from DLL import alone.
- [ ] Executable license status remains `UNKNOWN` in #613; no executable license command is run because no current-repo `-check-license` or equivalent support is verified.
...
- [ ] Advisory checks can set `overall_status=FAIL`, `WARN`, or `UNKNOWN` while the process exit code remains 0 unless a `--require-*` flag is unmet; status aggregation uses `FAIL > WARN > UNKNOWN > PASS`, and requirement flags include explicit coverage for `--require-executable` and `--require-license`.
...
| Gemini | APPROVE | Verified UNKNOWN license handling, output-dir failure semantics, runner-detection separation, and CLI-owned dry-run warning; no blockers |
...
- **Risk:** The doctor must not imply a full license is available unless OrcFxAPI/executable exposes a reliable check. Unknown license state remains `UNKNOWN` in #613 and is aggregated below `WARN` but above `PASS`.

...
What remains (from the issue body, sections "1", "2", "3"): (1) end-to-end OrcaFlex `LoadData` + `CalculateStatics` validation on a converted YAML, (2) round-trip binary-fidelity test through the OrcaFlex binary format, (3) update the backward-compat wrapper if step (2) reveals divergences. All three require an OrcFxAPI license and must run on `ace-win-1`.
...
On `ace-win-1`, run the converter against a representative input:
```
...
Add `tests/solvers/orcaflex/format_converter/test_license_validation.py` with two tests, both decorated `@pytest.mark.requires_orcaflex`, encoding the Task 1 + Task 2 procedures. Configure the marker in `pyproject.toml` so dev-primary CI skips them and `ace-win-1` CI runs them. The fixture YAMLs already exist under `docs/modules/orcaflex/examples/raw/`.
...
- [ ] On `ace-win-1`, `uv run pytest -m requires_orcaflex tests/solvers/orcaflex/format_converter/` is green; both tests run.

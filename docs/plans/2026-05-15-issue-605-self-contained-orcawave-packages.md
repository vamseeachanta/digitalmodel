...
- [ ] Package publish semantics are manifest-owned: unrelated output files are not deleted, a failed first run leaves no package, a failed pre-existing-empty-dir publish restores the empty directory, and a failed rerun preserves the previous valid package.

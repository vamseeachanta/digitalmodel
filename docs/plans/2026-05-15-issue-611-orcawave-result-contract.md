...
| `test_subprocess_stdout_stderr_persisted_to_log_file` | subprocess telemetry is not lost | mocked executable writes stdout/stderr | output log file exists, `RunResult.artifacts.log_file` points to it, and manifest artifacts serialize it |
...
| `test_cli_export_xlsx_absent_preserves_spec_outputs_precedence` | absent flag is tri-state, not false | `run-orcawave` with neither export flag and spec `outputs.formats=["xlsx"]` | CLI passes `None`, runner exports `.xlsx` from spec output intent |
...
| `test_orcawave_report_data_adapter_returns_diffraction_results_for_batch` | batch post-processing remains type-complete | mocked `DiffractionReportData` from `.owr` plus manifest metadata | adapter returns `DiffractionResults` or batch contract is explicitly changed with tests; raw dict/`DiffractionReportData` is not passed where `DiffractionResults` is required |
...
| `test_batch_postprocessing_requests_xlsx_or_skips_explicitly` | default post-processing cannot assume Excel exists | batch post-processing path with default run config and xlsx-dependent conversion requested | batch sets `export_xlsx=True` for that run or emits a clear skipped-postprocess warning before calling the converter |
...
| `test_runner_report_artifact_helper_warns_without_orcfxapi` | report generation is optional on Linux/subprocess paths | report requested but `extract_report_data_from_owr` raises `ModuleNotFoundError` | warning recorded, `report_file=None`, existing run success/artifacts remain valid |
| `test_manifest_report_rewrite_is_atomic_and_preserves_fields` | post-run report update cannot corrupt manifest | manifest with status/export/thread/artifact fields then report helper update | temp-file replace is used and all pre-existing fields survive after reload |
...
- [ ] Successful runs expose `.owr`, saved data, optional `.xlsx`, report, persisted text log, and manifest paths in structured metadata.
...
- [ ] CLI export flags preserve tri-state behavior: absent flag passes `None`, `--export-xlsx` passes `True`, and `--no-export-xlsx` passes `False`.
- [ ] Batch or other xlsx-dependent post-processing either requests `.xlsx` through the artifact contract before the run or skips with a warning; it does not assume default runs produce Excel.
...
- [ ] Subprocess stdout/stderr are persisted to a text log when captured, and `RunResult.artifacts.log_file` / manifest artifacts point only at text logs, never `.owr`.
...
- [ ] Batch post-processing either receives a real `DiffractionResults` via an explicit adapter from `DiffractionReportData`/manifest or has its result type contract changed in the same PR; raw Excel-converter path dictionaries and raw `DiffractionReportData` are not passed into `DiffractionResults` consumers.
- [ ] Optional report generation records a warning, not a hard failure, when OrcFxAPI/report extraction dependencies are unavailable; manifest rewrites after report generation are atomic and preserve existing fields.
...
| Gemini | UNAVAILABLE | Final rerun hit Gemini quota after prior MAJOR findings were patched; artifact records provider failure |

**Overall result:** READY FOR USER REVIEW (best-effort) - completed artifacts exist, prior MAJOR findings were addressed by plan revisions, and the final rerun was provider-unavailable due quota.

# Plan: digitalmodel #603 - Add regression coverage for GoM tieback spec-to-modular pipeline
...
Add regression tests that lock the intended GoM tieback path:

```text
GoM tieback domain spec.yml -> jumper calculations -> generator-ready ProjectInputSpec
...
- Acceptance criteria require both GoM tieback specs, 27-section assertions, parseable generated YAML,
...
integration branch after those code slices, proves the two GoM tieback specs travel through the
...
| Artifact | Path |
|---|---|
| Plan | `docs/plans/2026-05-15-issue-603-gom-tieback-jumper-pipeline-regression.md` |
...
| Regression tests | `tests/solvers/orcaflex/modular_generator/test_gom_tieback_jumper_pipeline.py` |
...
6. Add a negative test documenting the current failure mode:
   - Direct `ProjectInputSpec(**yaml.safe_load(gom_tieback_spec))` raises a validation error containing
   ...
   - Preferred #602 behavior is non-mutating conversion, so this negative test should remain while the
     source GoM tieback files are domain specs.
...
7. Add an explicit no-hard-dependency regression:
   ...
   - At the top of the subprocess code, before importing any `digitalmodel` module, install a
     ...
     failure condition is that the GoM tieback conversion/generation path cannot complete when OrcFxAPI
     raises `ImportError`.
   - In that same fresh subprocess, loop over both GoM tieback domain specs, run
     ...
     `ModularModelGenerator.from_spec(...).generate(Path(os.environ["GOM_TIEBACK_TEST_TMP"]) /
     spec_path.parent.name)`.
   - Launch the subprocess with `cwd` set to the repository root, environment copied from
     `os.environ`, `PYTHONPATH` prepended with `src`, and `GOM_TIEBACK_TEST_TMP` set to the pytest
   ...
   - Assert the subprocess exits 0 and its stdout records both spec names. This proves the Linux code
     path for both GoM tieback variants has no hard dependency on the licensed OrcaFlex Python API. Do
...
8. Include the verification command in the test module docstring or closeout comment:
   ...
   ```bash
   PYTHONPATH=src uv run python -m pytest tests/solvers/orcaflex/modular_generator/test_gom_tieback_jumper_pipeline.py -q
   ```
...
| Action | Path | Reason |
|---|---|---|
| Add | `tests/solvers/orcaflex/modular_generator/test_gom_tieback_jumper_pipeline.py` | Regression coverage for the full GoM tieback spec-to-generator path |
...
|---|---|---|
| `test_gom_tieback_specs_calculate_27_orcaflex_sections` | current code | both specs report 27 sections |
| `test_gom_tieback_specs_direct_projectinputspec_failure_is_documented` | current code | direct domain-spec validation fails with the known model-type error |
| `test_build_modular_spec_validates_for_gom_tieback_specs` | #602 | converted dict validates as `ProjectInputSpec` |
...
| `test_generated_gom_tieback_model_tree_parses` | #602 | all generated YAML files parse |
| `test_generated_gom_tieback_model_contains_jumperline_and_environment` | #602 | generated `20_generic_objects.yml` contains `JumperLine`, 27 rows, and source water data |
| `test_generated_gom_tieback_line_type_references_resolve` | #602 | all line references resolve |
| `test_generated_gom_tieback_model_tree_has_expected_includes` | #602 | `01_general.yml`, `03_environment.yml`, and `20_generic_objects.yml` are included and present |
| `test_pipeline_line_sections_file_exists_for_gom_tieback_specs` | #601 | `run_pipeline(..., generate_report=False, run_go_no_go=False)` writes `line_sections.yml`; parsed YAML has root `line_sections` and 27 entries, proving the `orcaflex_sections_yaml` source key is wired |
| `test_gom_tieback_generation_does_not_require_orcfxapi` | #602 | conversion/generation works when `OrcFxAPI` imports raise `ImportError` |
...
- [ ] Test loads both GoM tieback specs from `docs/domains/orcaflex/subsea/jumper/installation/`.

# Plan: digitalmodel #604 - Validate generated GoM tieback jumper model on licensed OrcaFlex runner
...
Run the generated GoM tieback jumper model on a licensed OrcaFlex environment, preferably
...
- Acceptance criteria require generating both GoM tieback models, loading `master.yml` with OrcFxAPI on
...
- This issue is domain-specific for GoM tieback jumper generated models and should reuse queue proof
...
- `docs/plans/2026-05-15-issue-602-gom-tieback-jumper-projectinputspec-conversion.md` - local draft
  generator-ready model precursor created in this workspace; it will not be visible on GitHub `main`
  until committed/pushed.
- `docs/plans/2026-05-15-issue-603-gom-tieback-jumper-pipeline-regression.md` - local draft Linux
...
reports OrcFxAPI/license unavailable and #602 has not yet generated GoM tieback `master.yml` artifacts.
This issue exists to capture that licensed proof after prerequisites land.

## Deliverable

A validation report records, for both generated GoM tieback jumper models, whether licensed OrcaFlex can
...
| Artifact | Path |
|---|---|
| Plan | `docs/plans/2026-05-15-issue-604-gom-tieback-licensed-orcaflex-validation.md` |
...
| Generated model root | `output/gom_tieback_mf_plet/` and `output/gom_tieback_plet_plem/` or run-specific temp path |
| Validation report | `docs/reports/2026-05-15-gom-tieback-jumper-orcaflex-validation.md` or dated closeout path |
| Validation helper | `scripts/orcaflex/validate_gom_tieback_jumper_models.py` |
| Report-schema test | `tests/solvers/orcaflex/modular_generator/test_gom_tieback_validation_report.py` |

## Preconditions

- #602 has produced generator-ready GoM tieback `master.yml` and includes for both specs.
...
1. Add a tiny report validator/test before the licensed run:
   ...
   - Implement the report writer/parser in `scripts/orcaflex/validate_gom_tieback_jumper_models.py`
   ...
   - Keep this as local validation/reporting coverage; do not mock OrcFxAPI as proof of licensed
     behavior.
2. Generate both GoM tieback models from repository code after #602 lands:
...
4. On licensed Windows, run a direct load/export smoke through
   `scripts/orcaflex/validate_gom_tieback_jumper_models.py`. If the helper cannot be used on the target
   ...
   failures = 0
   for master in [
       repo / 'output/gom_tieback_mf_plet/master.yml',
       repo / 'output/gom_tieback_plet_plem/master.yml',
   ]:
       row = {
           'commit': commit,
           'host': host,
...
5. Record source-data provenance before interpreting a successful load:
   - MF-PLET can be reported as a generated GoM tieback MF-PLET load/export proof.
...
6. If load/export succeeds and statics is in scope for the generated model, run `CalculateStatics()`
   ...
   a `--run-statics` flag or `GOM_TIEBACK_RUN_STATICS=1`; default validation is load/export only. Do
...
| Action | Path | Reason |
|---|---|---|
| Add | `docs/reports/YYYY-MM-DD-gom-tieback-jumper-orcaflex-validation.md` | Durable licensed validation evidence |
| Add | `docs/reports/YYYY-MM-DD-gom-tieback-jumper-orcaflex-validation.json` | Structured report consumed by tests and rendered to Markdown |
| Add | `scripts/orcaflex/validate_gom_tieback_jumper_models.py` | Repeatable direct OrcFxAPI load/export helper plus structured report writer/parser |
| Add | `tests/solvers/orcaflex/modular_generator/test_gom_tieback_validation_report.py` | Report shape and required-field coverage |

## TDD Test List

| Test | Expected RED | Expected GREEN |
|---|---|---|
| `test_gom_tieback_validation_report_requires_commit_host_and_model_rows` | report parser/helper absent | report contains required metadata and one row per model |
| `test_gom_tieback_validation_report_requires_failure_followup_links` | missing failure follow-up allowed | failed model rows require a follow-up issue URL |
| `test_validate_script_lists_both_expected_master_paths` | helper absent | helper enumerates both GoM tieback `master.yml` paths |
...
- [ ] GoM tieback MF-PLET model YAML is generated from repo code.
- [ ] GoM tieback PLET-PLEM model YAML is generated from repo code.

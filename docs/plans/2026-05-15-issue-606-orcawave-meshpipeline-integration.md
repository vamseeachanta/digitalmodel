...
> **Review artifacts (target paths; created by completed fanout, not pre-existing evidence):** scripts/review/results/2026-05-15-plan-606-claude.md | scripts/review/results/2026-05-15-plan-606-gemini.md

---

## Scope

Mesh format preparation only. This issue should build on #605 package layout and #608 quality gates, but it does not define the given-mesh CLI (#607). It must reuse the #605/#500 shared asset resolver so generated meshes are not later overwritten or skipped by the legacy runner copy path.
...
2. Add a mesh preparation service that short-circuits ready GDF assets before invoking `MeshPipeline`, calls `MeshPipeline.prepare_for_solver()` only for supported conversion classes, and returns an `OrcaWavePreparedAsset` manifest for generated and passthrough assets.
3. Hoist path resolution before backend YAML generation, then prepare meshes and generate YAML from an explicit prepared-filename map keyed by asset id/type; do not mutate `self.spec` and do not use a deep-copied spec as the primary API because `convert_all()` reuses `self.spec` across AQWA and OrcaWave.
4. Update all backend mesh filename write sites through the non-mutating prepared-filename map: `BodyMeshFileName`, `BodyControlSurfaceMeshFileName`, `DampingLidMeshFileName`, and `FreeSurfacePanelledZoneMeshFileName`.
5. Refactor runner copy/validation so generated prepared assets are not resolved back against `spec_dir` and cannot overwrite converted output with an older source file of the same basename.
6. Preserve units/symmetry/source provenance in run metadata and add tests for passthrough, conversion, unsupported format, damping lid, FSZ `.fdf`, runner overwrite protection, and generated YAML filename updates.
...
| Modify | `src/digitalmodel/hydrodynamics/diffraction/orcawave_backend.py` | accept prepared mesh filenames via deep-copied spec or explicit map, without mutating shared converter state |
...
|---|---|---|---|
| `test_orcawave_mesh_prepare_gdf_passthrough` | no churn for ready mesh | `.gdf` body/control/damping-lid mesh | same basename packaged without invoking `MeshPipeline.write()` |
| `test_orcawave_mesh_prepare_dat_to_gdf` | supported conversion path | `.dat` mesh | `.gdf` output referenced by YAML |
| `test_orcawave_mesh_prepare_unsupported_format_fails` | clear error surface | `.obj` body mesh | supported-format error |
...
- [ ] The implementation plan has one shared path-resolution/copy path with #605/#500; no parallel resolver remains in both runner and preparer.

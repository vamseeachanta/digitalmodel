...
7. Return and persist an `OrcaWaveMeshPreparationManifest` at `orcawave_mesh_manifest.json` with `prepared_assets` and top-level `warnings`. Each `OrcaWavePreparedAsset` contains `asset_id`, `asset_type`, `source_path`, `output_path`, `yaml_filename`, `yaml_mesh_format`, `yaml_length_units`, `yaml_symmetry`, `source_format`, `target_format`, `was_passthrough`, `was_converted`, and conversion warnings. Deferred or non-asset findings, especially pre-#609 `BodySpec.control_surface` mesh deferrals that do not produce `prepared_assets`, must be written to top-level manifest warnings with `asset_id`, `source_field`, `severity`, `code`, and `message` so they survive beyond stdout/CLI rendering. The mesh manifest is a child artifact of the #605 package: it must be written inside the #605 staging directory and listed in `orcawave_package_manifest.json` alongside prepared mesh files through the #606-owned extension to #605's manifest writer so package replacement/cleanup treats converted assets as owned files.
...
| `test_orcawave_control_surface_native_format_uses_guarded_metadata` | control-surface native formats are representable without crossing #609 scope | selected vessel-level `.csf` or `.gdf` `VesselSpec.control_surface` with supported metadata | prepared metadata emits correct filename/format/units, or clear deferral before YAML if #609-selected semantics are not yet available; `.obj`/`.msh` remain fail-closed |
| `test_orcawave_mesh_prepare_has_no_undefined_force_gdf_policy` | conversion policy surface is not invented by tests | `.dat` body/damping fixture without a declared conversion-policy API | native passthrough emits `Aqwa dat`; no `force_gdf`/conversion flag is required or referenced by #606 |
...
- [ ] Deferred `BodySpec.control_surface` warnings that produce no prepared asset are persisted in top-level `orcawave_mesh_manifest.json` warnings with source-field provenance.
...
| Gemini | MINOR | Notes #605 dependency, existing backend format drift, modular-output follow-up risk, and MeshPipeline API ambiguity; no unresolved MAJOR |

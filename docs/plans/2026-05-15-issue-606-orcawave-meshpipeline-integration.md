...
3. Define format policy explicitly: OrcaWave-native `.gdf`, `.obj`, and `.msh` assets are solver-ready passthroughs with matching YAML mesh format. Control-surface native passthrough requires adding optional `mesh_format` and `length_units` fields to `ControlSurfaceSpec`; otherwise the preparer must normalize to a backend-representable GDF/CSF path. `.dat` and `.stl` use `MeshPipeline` to produce GDF when supported; unknown formats such as `.iges` fail with `OrcaWaveMeshPreparationError`.
...
6. Use collision-safe prepared filenames for converted assets: include `asset_type` and a stable `asset_id` prefix (for example `body_0__hull.gdf`, `body_0_control_surface__hull.gdf`) so same-stem body/control/damping assets cannot overwrite each other. This requires either extending `MeshPipeline.prepare_for_solver()` with an optional output-name/path override or using its lower-level load/convert/write steps; the current fixed `stem + target_ext` behavior is not sufficient. Passthrough basename collision policy remains owned by #605.
...
10. Preserve units/symmetry/source provenance in `orcawave_mesh_manifest.json`. `RunResult.mesh_files` remains a `list[Path]` compatibility surface that records prepared output paths only; #611 may later promote manifest fields into the general run manifest, but #606 must not depend on #608 or #611.
...
prepare_orcawave_meshes(spec, spec_path, output_dir):
  ...
  for asset in assets:
    ...
    elif asset ext in {.dat,.stl}: convert via MeshPipeline with explicit collision-safe output name
...
| Modify | `src/digitalmodel/hydrodynamics/diffraction/mesh_pipeline.py` | allow explicit converted output filename/path or expose a safe lower-level write path |
| Modify | `src/digitalmodel/hydrodynamics/diffraction/input_schemas.py` | add optional control-surface mesh format/units only if native control-surface passthrough is in scope |
...
| `test_orcawave_control_surface_native_format_requires_schema_metadata` | control-surface native formats are representable | `.obj` control surface | either schema metadata emitted correctly or clear deferral/error before YAML |
...
| `test_orcawave_mesh_prepare_body_control_surface_rejected_until_609` | known body-level field edge is explicit | `BodySpec.control_surface.mesh_file` | clear #609 deferral/error, no silent miss |
...
- [ ] OrcaWave-native `.gdf`, `.obj`, and `.msh` body/damping assets pass through with correct mesh-format metadata; control-surface native passthrough is either supported by explicit schema metadata or rejected/deferred clearly; supported conversion formats produce GDF; unknown formats fail with a domain-specific supported-format list.

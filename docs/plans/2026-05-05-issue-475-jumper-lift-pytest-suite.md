...
6. **Add pipeline integration test.** `test_pipeline_mf_plet_end_to_end`:
   ...
   def test_pipeline_runs_mf_plet_end_to_end(tmp_path):
       ...
       cfg = PipelineConfig(spec_path="docs/domains/orcaflex/subsea/jumper/installation/gom_tieback_mf_plet/spec.yml",
...
- [ ] Both `gom_tieback_mf_plet` and `gom_tieback_plet_plem` are exercised (parametrize where applicable)

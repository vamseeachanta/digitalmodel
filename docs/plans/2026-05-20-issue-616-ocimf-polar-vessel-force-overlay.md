...
- `workspace-hub#2760` — OPEN — `status:plan-review` — "revise(naval-arch): <job-code> tanker_225m_1 force calculation review updates" — confirms downstream-consumer status.
...
| New smoke test (cross-consumer) | `digitalmodel/tests/marine_ops/marine_engineering/visualization/test_polar_force_overlay_smoke_tanker_225m_1.py` |
...
| Create | `tests/marine_ops/marine_engineering/visualization/test_polar_force_overlay_smoke_tanker_225m_1.py` | Cross-consumer smoke test #14 (strengthened per r1 m2) |
...
| 14 | `test_smoke_tanker_225m_1_consumer_readiness` | **r1 m2 strengthened**: synthetic tanker_225m_1-style force DataFrame (X/Y/Z/K/M/N at rudder ±5°) produces a fully-readable figure: (a) returns `go.Figure`; (b) figure has exactly 6 distinct legend entries (one per component); (c) for at least 3 of the 6 components, the resolved arrow directions are all *distinct* (no collapsed/coincident arrows); (d) no exceptions, no warnings; (e) figure layout has non-empty title and angularaxis ticks. If an anonymized tanker_225m_1 fixture becomes available, swap the synthetic shape for it. | wide-format df with `(theta_deg, fx, fy, fz, mx, my, mz)` at 11 rudder-angle headings | all five sub-conditions above pass |
...
- **Risk (silhouette accuracy):** generic tanker/gas-carrier silhouettes may visually mismatch the specific vessel a study models (e.g., <job-code> tanker_225m_1). Mitigation: support `silhouette_kind='custom_path'` with caller-supplied polygon (test #11).
...
- **Open:** for tanker_225m_1 consumer (#2760), should the arrows reflect the *rudder-angle-induced* component split (separate arrows per rudder angle) or only the total resultant? This is a #2760-side decision; this module's API supports either via `group_col`.

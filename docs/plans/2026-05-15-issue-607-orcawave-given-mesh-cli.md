...
2. Define concrete `mesh-to-spec` CLI flags: `--name` optional with default `mesh.stem`, `--mass`, `--cog x,y,z`, exactly one of `--rog x,y,z` or `--inertia ixx,iyy,izz,ixy,ixz,iyz` after template merge, `--water-depth`, `--frequencies f1,f2,...` or `--frequency-range min,max,count`, `--frequency-input-type frequency|period` defaulting to `frequency` and documented as rad/s for `frequency` and seconds for `period`, `--frequency-distribution linear|logarithmic` defaulting to `linear`, `--headings h1,h2,...` or `--heading-range min,max,increment`, `--mesh-format auto|gdf|dat|stl`, `--mesh-units` optional with effective default `m` only when no template value exists, `--symmetry none|xz|yz|xz+yz` with `both` accepted only as a CLI alias translated to `xz+yz`, `--template`, `--body-index` for selecting a body in complete multi-body templates, `--keep-invalid`, and `--output` as the generated spec path with default `<mesh.stem>.diffraction.yml` in the current working directory. Internally distinguish explicit CLI values from effective defaults: explicit `--mesh-units` overrides template `length_units`, but an omitted option preserves a template's non-`m` units and only fills `m` when the merged geometry has no units. Build and Pydantic-validate the merged `DiffractionSpec` before writing canonical YAML; schema construction failures exit nonzero without creating an output file, even when `--keep-invalid` is present. After schema validation and before `DiffractionSpec.to_yaml(path)`, create the output parent directory if it is missing; parent creation or write failures exit nonzero and remove any partial generated file unless `--keep-invalid` applies to a successfully serialized file that later fails converter validation. `--keep-invalid` is an explicit #607 contract for files that have already been serialized successfully: when post-write converter validation fails, the command exits nonzero and keeps the generated file only if this flag is present; otherwise it removes the generated file.
3. Define `run-orcawave-from-mesh` as a wrapper over the `mesh-to-spec` builder using the same mesh, physical-input, template, body-targeting, validation, and `--keep-invalid` flags except `--output`. To follow adjacent Click conventions, the wrapper uses `--output/-o` for the OrcaWave dry-run output directory and exposes `--spec-output` for the generated spec path. If `--spec-output` is omitted, write `<output>/<mesh.stem>.diffraction.yml` so the runner's spec-relative mesh copy convention is deterministic. It must not introduce separate mesh handling; internally it calls the builder and then the existing runner.
4. Define mesh-format inference explicitly: with `--mesh-format auto`, suffix matching is case-insensitive and maps `.gdf -> gdf`, `.dat -> dat`, and `.stl -> stl`; `.obj` and `.msh` fail closed in #607 to stay aligned with #606 until handlers and source-backed policy land. Unknown suffixes fail unless an explicit supported `--mesh-format` is provided. This suffix table is schema-value inference for authoring a `DiffractionSpec`, not a path resolver and not a `MeshPipeline.detect_format()` replacement. #607 only writes schema/YAML values and can dry-run/copy the referenced mesh through the current runner; it does not prove licensed OrcaWave acceptance for `.stl`. #606 owns preparation/conversion policy and #610 owns licensed acceptance.
...
| `test_mesh_to_spec_default_output_path` | omitted output is deterministic | mesh + required numeric options without `--output` | spec is written to `<mesh.stem>.diffraction.yml` under the current working directory |
...
| `test_mesh_to_spec_obj_msh_fail_closed_until_606_supports_them` | #607 does not accept formats #606 forbids | `.obj` or `.msh` mesh with `--mesh-format auto` or explicit format | Click error names unsupported format and points to #606/#610 for future conversion/licensed acceptance |
...
| `test_mesh_to_spec_creates_output_parent_after_validation` | parent handling is deterministic | valid input with `--output nested/generated.yml` where `nested/` is absent | parent directory is created after schema validation and spec writes successfully |
| `test_mesh_to_spec_write_failure_cleans_partial_file` | write failures do not leave misleading specs | mocked `DiffractionSpec.to_yaml()` write failure after parent creation | nonzero exit and no partial generated file remains |
...
| `test_run_orcawave_from_mesh_thin_wrapper` | issue-body command shape does not duplicate logic | ready mesh plus physical flags, `--output/-o`, and optional `--spec-output` | wrapper writes the generated spec, invokes the same runner path, and produces `RunStatus.DRY_RUN`; `--output` is the dry-run directory and `--spec-output` is the spec path |
...
- [ ] `run-orcawave-from-mesh` is included as a dry-run-only thin wrapper around `mesh-to-spec` plus the existing runner, uses `--output/-o` for the dry-run directory, exposes `--spec-output` for the generated spec path, uses a deterministic `<output>/<mesh.stem>.diffraction.yml` default when `--spec-output` is omitted, and introduces no separate mesh resolver/converter.
...
- [ ] `.stl` is documented as schema/YAML dry-run input only in #607; `.obj` and `.msh` fail closed until #606/#610 add handlers/source-backed conversion and licensed acceptance.
...
| Gemini | APPROVE | Reviewed CLI scope, fail-closed mesh formats, output semantics, and docs dependency; no blockers |

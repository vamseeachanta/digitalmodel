# Plan: digitalmodel #602 - Convert GoM tieback jumper specs into generator-ready ProjectInputSpec models
...
Implement a conversion layer that reads the GoM tieback jumper installation domain specs and builds a
...
`ProjectInputSpec(**data)` for both GoM tieback jumper specs, and `ModularModelGenerator.from_spec()`
...
| Artifact | Path |
|---|---|
| Plan | `docs/plans/2026-05-15-issue-602-gom-tieback-jumper-projectinputspec-conversion.md` |
...
1. Add RED tests before creating the conversion module:
   - `test_build_modular_spec_validates_for_gom_tieback_specs`
   - `test_build_modular_spec_preserves_27_line_sections`
   - `test_modular_generator_writes_master_and_includes_for_gom_tieback_specs`
   - `test_gom_tieback_specs_are_distinct_or_baseline_geometry_is_explicitly_marked`
...
6. Convert the computed 27 sections into the compact OrcaFlex multi-column line table inside a
   ...
   - `PreBendCurvaturex`: 0 for all current GoM tieback rows unless source data adds out-of-plane
     curvature.
   - `PreBendCurvaturey`: nonzero for bend rows using `1 / bend_radius_m` with this provisional
     GoM tieback sign map, derived from the existing rigid-jumper template pattern and recorded in
   ...
   count/key state.
7. Map GoM tieback environment data into `ProjectInputSpec.environment`:
...
{
    "metadata": {
        "name": "gom_tieback_mf_plet_jumper",
        "description": "GoM tieback Manifold-to-PLET Jumper Installation Analysis",
        "structure": "installation",
        "operation": "installation/lift",
        "project": "GoM tieback",
        "version": "1.0",
    },
    # Example for GoM tieback specs with seabed data present. If a future spec omits the whole seabed
...
`te/m3`, while the GoM tieback source insulation density is `kg/m3` and must be divided by 1000.
...
```python
{
    "source_spec": "docs/domains/orcaflex/subsea/jumper/installation/gom_tieback_mf_plet/spec.yml",
...
For `gom_tieback_plet_plem`, `baseline_geometry` must be derived from source text/comments rather than
...
|---|---|---|
| `test_build_modular_spec_validates_for_gom_tieback_specs` | import/function missing | `ProjectInputSpec(**data)` succeeds for both specs |
| `test_build_modular_spec_preserves_27_line_sections` | missing conversion | one generated line contains 27 section rows |
| `test_modular_generator_writes_master_and_includes_for_gom_tieback_specs` | generator cannot run | `master.yml`, `inputs/parameters.yml`, includes exist and parse |
...
| `test_gom_tieback_specs_are_distinct_or_baseline_geometry_is_explicitly_marked` | current ambiguity not asserted | differences are asserted, or baseline reuse is documented in metadata |

## Acceptance Criteria

- [ ] `build_modular_spec()` or equivalent returns dicts accepted by `ProjectInputSpec(**data)` for
  both GoM tieback specs.
...
for spec_path in [
    ...
    out = Path('/tmp/gom-tieback-modular') / spec_path.parent.name
    if out.exists():
        shutil.rmtree(out)

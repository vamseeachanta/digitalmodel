# Issue Plans

This is the active issue-plan index for `digitalmodel`. Historical plans in
this directory predate this index and are not backfilled here.

| Issue # | Title / Slug | Plan File | Date | Status | Complexity | Notes |
|---|---|---|---|---|---|---|
| [#1602](https://github.com/vamseeachanta/digitalmodel/issues/1602) | solver-neutral-riser-data-orcaflex-v1-hf-program | `docs/plans/2026-07-16-issue-1602-riser-data-orcaflex-hf-plan.md` | 2026-07-17 | plan-approved | T3 | Owner approved a generic solver-neutral SSOT with replaceable solver adapters. Interface v2 draft 11 keeps OrcaFlex as v1, binds target semantic coverage/gap evidence as a pre-execution terminal gate, prohibits every native solver model format from HF, and passed unanimous Round 16 review. Child plans retain separate approval gates. |
| [#1470](https://github.com/vamseeachanta/digitalmodel/issues/1470) | registry-batch-coverage-wave-6 | `docs/plans/2026-07-08-issue-1470-registry-batch-coverage.md` | 2026-07-08 | plan-approved | T3 | Provider CLIs unavailable; two subagent reviews returned MAJOR, plan was revised, focused re-reviews returned APPROVE. User approved; implementation authorized with TDD. |

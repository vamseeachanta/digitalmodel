...
log_message "normal" "Starting repository check process..."

# Iterate through all directories in the GitHub folder
for dir in "$github_dir"/*/ ; do
    if [ -d "$dir" ]; then
        # - [ ] todo - make these checking white 
        log_message "green" "Checking for changes in folder: $dir"
        cd "$dir"
        ...
        if [ -n "$(git status --porcelain)" ]; then
            ...
            if [ -f "$daily_routine_script" ]; then
                log_message "green" "Running daily routine script..."
                bash "$daily_routine_script"
                log_message "green" "Daily routine completed"
            else
